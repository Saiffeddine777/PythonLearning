...
only count as part of a triplet (contributing to the 500 points) or as a single 50 points, 
...
def score(dice):
    noDuplicates =[]
    for e in dice:
         if e not in noDuplicates:
              noDuplicates.append(e)
    print(noDuplicates)
    score = 0
        ...
        if e in [2,3,4,6]:  
            ...
            if dice.count(e)>=3:
             ...
             if number in [1,2]:
                  ...
                  if e==1:
                       scoreofE= 1000 + (number-3)*100
                  if e== 5: 
                       scoreofE= 500 + (number-3)*50
